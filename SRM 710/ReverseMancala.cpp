...
vector <int> build(vector <int> a, int op) {
	vector<int> ans;
	int s = 0, i, p;
	for(i = 0; i < a.size(); i++) s += a[i];
	while(a[0] != s) {
		for(i = 1; !a[i]; ++i);
		ans.push_back(op ? (i + a[i]) % n + n: i);
		for(p = a[i], a[i] = 0, i = (i + 1) % n; p; --p, ++a[i], i = (i + 1) % n);
	}
	return ans;
}
...
class ReverseMancala {
    ...
    vector<int> findMoves(vector<int> start, vector<int> target) {
		...
		vector<int> pa = build(start, 0);
		vector<int> pb = build(target, 1);
		
		for(int i = pb.size() - 1; ~i; i--) 
			pa.push_back(pb[i]);
